...
import (
	"fmt"
	c01 "geek/03.module/calc"
	c02 "geek/03.module/calc_upgraded"
	"os"
	"runtime/debug"
)
...
func calcFatRate(weight float64, tall float64, age int, sex string) (fatRate float64, err error) {
	// 计算体脂率
	bmi, err := c01.CalcBMI(tall, weight)
	if err != nil {
		return 0, err
	}

	fatRate, err = c02.CalcFatRate(bmi, age, sex)
	fmt.Println("体脂率是：", fatRate)
	return
}

func getMaterialsFromInput() (name1 float64, weight1 float64, tall1 int, age1 int, sex1 string) {
	// 录入各项
	var name string
	fmt.Print("姓名：")
	fmt.Scanln(&name)
